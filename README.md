...
**Back / Forward Navigation (v1.5.0)**

New ◀ ▶ buttons at the bottom of the sidebar, or use `⌘[` / `⌘]` shortcuts for page navigation.

**Custom Prompt Prefix (v1.6.0)**

Each Destination can have a custom prompt prefix that is automatically prepended when sending text via `⌘C ⌘C`. For example, set the prefix to "Translate the following text to English:" and any copied text sent to Gemini will automatically include the translation instruction.

Configure in: ⚙ Settings → Edit Destination → Prompt prefix field
...
- ⚡ **非同步凍結**：背景頁面的凍結操作改為非同步執行，Gemini 等重度頁面卡住時不再阻塞整個 app
- 🧹 **背景頁面自動回收**：閒置超過 2 分鐘的背景頁面自動銷毀，釋放記憶體
- 📜 **側邊欄捲動**：Destination 圖示數量超出視窗高度時，側邊欄可捲動瀏覽

**v1.5.0 — 上一頁 / 下一頁導航**

- ◀▶ **頁面導航按鈕**：側邊欄底部新增上一頁 / 下一頁按鈕
- ⌨️ **快捷鍵支援**：`⌘[` 上一頁、`⌘]` 下一頁
...
- ⚡ **Async freeze/thaw**: Background page freezing now runs asynchronously, so a stuck page (e.g. heavy Gemini sessions) no longer blocks the entire app
- 🧹 **Background page auto-cleanup**: Pages idle for 2+ minutes in background are automatically destroyed to reclaim memory
- 📜 **Sidebar scroll**: Destination icons now scroll when they exceed the sidebar height

**v1.5.0 — Back / Forward Navigation**

- ◀▶ **Page navigation buttons**: New back/forward buttons at the bottom of the sidebar
- ⌨️ **Keyboard shortcuts**: `⌘[` to go back, `⌘]` to go forward
